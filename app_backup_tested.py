...
# Streamlit UI for Evaluation Metrics
def evaluation_metrics(nlp=None, test_sentences=None, test_tags=None):
    ...
    if nlp==None and test_sentences==None and test_tags==None:
        print("inside eval lvl 2")
        
        selected_model = st.selectbox("Select Model for Evaluation", nlp_models)
        
        # Load the selected spaCy model
        nlp = spacy.load(selected_model)
        
        st.subheader("Model Performance Metrics")
        st.write("TODO: Display metrics here")
        
        st.subheader("Evaluate on New Data")
        evaluate_new_data = st.checkbox("Evaluate on New Test Data")
        if evaluate_new_data:
            new_test_data = st.text_area("Enter New Test Data")
            if new_test_data:
                # Convert new test data to spaCy format
                new_test_examples = convert_to_spacy_format(new_test_data, new_test_tags)
                
                # Evaluate the model on new test data
                # TODO: Calculate metrics and display results

    elif nlp and test_sentences and test_tags:
        print("inside eval lvl 2")
        test_examples = convert_to_spacy_format(test_sentences, test_tags)
        # Evaluate the model on the validation set
        test_precisions = []
        ...
        metrics = nlp.evaluate(test_examples)
        test_precisions.append(metrics["ents_p"])
        test_recalls.append(metrics["ents_r"])
        ...
        st.warning('we need nlp model, test sentences as well as their corresponding labels', icon="⚠️") 
